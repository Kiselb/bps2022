...
test('Тестирование консистентности Identity шагов wizard (страниц)', () => {
    ...
    for(let i = 0; i < automaton.length; i++) {
        ...
        if (pages !== null) {
            for(let j = 0; j < pages.length; j++) {
                ...
                types.push(pages[j].type.toUpperCase())
    ...
    identities = identities.filter((value, index, array) => {
        return array.indexOf(value) === index;
    });
    types = types.filter((value, index, array) => {
        return array.indexOf(value) === index;
    });
      
    //console.log(identities.sort());
...
//         const pages = automaton[i][3];
//         if (pages !== null) {
//             let charges: WizardStageCharges[] = [];
//             let chargescheck = [];
//             for(let j = 0; j < pages.length; j++) {
//                 const page = pages[j];
